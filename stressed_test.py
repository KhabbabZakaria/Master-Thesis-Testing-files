...
#print(price_bar)
price_bar = price_bar_stressed_test
...
twap_price = np.array(twap_price).reshape(len(all_stressed_test_list), T)
vwap_price = np.array(vwap_price).reshape(len(all_stressed_test_list), T)
weight_vwap_list = np.array(weight_vwap_list).reshape(len(all_stressed_test_list), T)
...
PA_positve = 0
PA_negitve = 0
for i in range(len(all_stressed_test_list)):
    P_bar_strategy = 0 #AEP
    PA_internal = 0
    for j in range(len((all_stressed_test_list[i]))):
...
total_reward = total_reward/(len(all_stressed_test_list)*T)
print('TWAP total_reward', total_reward)

PA = 10**2/len(all_stressed_test_list) * PA
print('TWAP PA', PA)
...
for k in range(1,2):
    ...
    for i in range(len(all_stressed_test_list)):
        ...
        PA_internal = 0
        env.reset()
        state, _ = getState(all_stressed_test_list[i], all_stressed_test_vwap_list[i], env.time, state_size)
        for j in range(state_size, len((all_stressed_test_list[i]))):
            price = all_stressed_test_vwap_list[i][j]
            ...
            env.step(action)
            next_state, _ = getState(all_stressed_test_list[i], all_stressed_test_vwap_list[i], env.time, state_size)
            total_reward = total_reward + reward
